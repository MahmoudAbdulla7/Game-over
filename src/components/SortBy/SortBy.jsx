...
export default function SortBy() {
    ...
    async function getCategories(type){
        let {data}= await axios.get(url ,{headers:headers ,params:{'sort-by':type}});
        setsrtBy(data);
    }
    useEffect(()=>{
        getCategories();
...
<Helmet>
    ...
    <div className="row gy-3">
          {srtBy.slice(0,seeMore).map((el,idx) => <MediaItem key={idx} el={el}/>
